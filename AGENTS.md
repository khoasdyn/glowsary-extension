...
8b. Color has two layers. Primitives live in `extension/color-tokens.js` (the raw palette, names `--color-{family}-{step}`). Semantic role tokens live in a separate file with short role names like `--text-primary`, `--bg-secondary`, and `--border-primary`, and each one references a primitive token, never a raw value. When you set a color in the UI, use a semantic role token where one fits the role, and use a primitive only when no role token covers the case.
