...
12b. NOTES.md is the project's running log of mismatches and open gaps (UI versus design, Figma versus the code tokens, request versus PRD.md, and similar). Read it for context on what is known to be unresolved, so you do not build on top of an open gap. The planning skills (product-planner and design-planner) own NOTES.md; like DESIGN.md, it is read-only for you. Never edit it.
