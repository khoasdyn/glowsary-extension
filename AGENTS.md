...
12a. Component styles always come from the Components section of DESIGN.md. That section is the single style guide for shared UI components (button, input, card, and so on). When you build or change a component, match its spec there and reference the tokens it names. Do not invent a component style that is not in that section; if one is missing or unclear, stop and tell the user so Claude adds it from Figma first.
