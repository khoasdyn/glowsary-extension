...
- Made the hover definition popup reliable on every site where a word is highlighted. Hover handling now lives on the document instead of on each underlined word, so the popup still appears on sites that rebuild, clone, or re-render the page (for example vietnamnews.vn) where the old per-word handlers were silently dropped. The popup also recovers its content if a word is rebuilt, and it now appears when the pointer is already resting on a word as its highlight shows up. Popup content, layout, position, sticky behavior, and pagination are unchanged.
