...
- The reader renders pages lazily as they scroll into view (with a one-screen lookahead) and caps canvas resolution at 1.5x, instead of rendering every page up front. Opening a long PDF is now near-instant and uses a fraction of the memory; a 57-page test went from ~6.3s and 57 rendered pages to about 0.1s and 2 rendered pages on open.
- Built the reader's popup and Add Word shadow root at load time so its styles are ready before the first hover or right-click, fixing the first Definition Popup appearing unstyled in the top-left corner.
- Fixed the reader's Add Word dialog clipping its lower fields (Aliases, Color, Save) with no way to scroll: the form now scrolls inside the dialog with the Save button kept in view, on short windows too.
- Fixed the active-word fill hiding the word inside a PDF: the fill is now translucent so the page's own text stays readable through its color tint, matching the active highlight on websites.
- Clicking a PDF cover opens a full-screen reader with a white close (X) button, zoom in/out, and page navigation. Saved words are highlighted in the PDF's real text exactly as on websites (same matcher, dashed underline, active fill, and Definition Popup with sound, image, multi-meaning paging, and edit). Right-clicking selected text in the reader offers "Add word" with the selection pre-filled.
- Uploaded PDFs and their covers are stored locally in the browser (IndexedDB), separate from saved words; they are not part of word import/export. A PDF can be removed from its card with no confirm and no undo, matching the rest of the app.
- Known limitation to refine later: multi-word phrases (for example "get in") are not highlighted inside a PDF when the PDF splits text one word per span, since matching runs per text node; single-word saved words highlight reliably. Scanned image-only PDFs have no text to highlight (FR-47g).
- Vendored Mozilla's PDF.js (v4.10.38) under `extension/vendor/pdfjs` to render PDFs locally, since the browser's built-in PDF viewer does not expose its text to the extension.
- Added a `file-04` document icon for the PDF tab and `plus`/`minus` icons for the reader's zoom controls. Loaded `content.css` into the management page so the reader can reuse the existing highlight and popup styles. No design tokens were added or changed.
