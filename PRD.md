...
- FR-3e: Every saved entry has one color, taken from a fixed set of four colors. The color is a visual tag for the entry’s card in the management list (see FR-17g). It is not an on-page highlight color: the highlight underline stays a single style for all words (FR-9) and the definition popup is unchanged. The color is chosen in both places that create or change an entry: the in-page save form (FR-2) and the Management form panel (FR-17f). In both, one color is always selected, and new entries start with the first color by default.
