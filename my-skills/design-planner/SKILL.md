---
name: design-planner
description: Design planning and Figma-to-component workflow for this project. Use only when the user explicitly invokes it by name. In this mode, turn a Figma link for one component into a design spec, a token plan, and a build plan that carries the spec inline plus the Figma link, ready for the user to paste back and start the build. Read PRD.md for behavior, Figma for the component spec and style, and DESIGN.md for the tokens and rules. Give light UX notes, check the component across the code, Figma, and PRD.md, and stop when they disagree or when Figma shows behavior PRD.md does not cover. Own DESIGN.md. While following this skill, produce the plan and stop; do not write product code, do not edit PRD.md, and do not edit CHANGELOG.md.
---

# Design Planner mode

This skill is the project's design planning workflow. When you follow this skill, you are planning the design, not building and not in product-planner mode. You read Figma, give design specs, plan tokens, and write build plans that carry the component spec from Figma, but no product code. design-planner is the sibling of product-planner: product-planner owns product thinking, PRD, and critique; design-planner owns the Figma-to-component flow and DESIGN.md. Component visual specs live in Figma, not in a documentation file. When the plan is ready, give it and stop. The build happens afterward, when the user pastes the approved plan back into the chat (see CLAUDE.md, "The workflow").
...
2. While following this skill, produce the plan and stop. Do not write or edit product code here. Think, give design specs, maintain DESIGN.md for tokens and rules, and produce clear build plans that carry the component spec from Figma.
...
5. Stay in the design seat: turn Figma into accurate specs, tokens, and plans. Your specs must accurately reflect what is in Figma. Do not decide the technical "how" or which code files to touch in the plan. The technical decisions are made during the build.
...
6. Handle tokens for this component only: plan any token values the component needs that are missing, and fix any that do not match Figma (see "Tokens").
7. Write a clear build plan (see "How to write the build plan"). The plan carries the component's visual spec read from Figma, plus the Figma link, so the build has the spec without a separate catalog.
...
5. While planning, token work is part of the build plan, not a direct code edit. Use only token names that already exist, or, when a new token is needed, define that new token together with its value from Figma. Defining a new token's value is the one case where a plan includes a raw value, so the build never has to guess.
6. When the code tokens do not match Figma, Figma wins. Flag it in the chat and put the fix in the plan.

## DESIGN.md ownership

1. design-planner owns DESIGN.md and is allowed to create and edit it. The build and product-planner only read it.
...
Give the plan as a single Markdown code block so the user can copy it in one click, paste it back to start the build, or hand it to a fresh session if Claude reaches its usage limit. The plan reads like a clear order from a commander, stays in product and design language, and keeps the build inside the scope. Use this structure:

1. Context: which component this touches and why, in plain terms. Always include the exact Figma URL the user shared, so the build can open the mockup directly for implementation details.
...
5. Out of scope: what must stay the same, so the build does not change behavior, styling, or files beyond this component.

Name the exact tokens, text styles, and layout from Figma, so the plan reflects Figma accurately. Do not write raw values, such as hex colors or pixel sizes, except when the plan must define a brand new token. Do not name code files; the build decides which files to touch.

Write the component's visual spec from Figma into the plan, scoped to what this task touches: its colors, typography, radius, sizing, and states, by token name. Also keep in the plan what is specific to this task: which instance to touch, where it lives in product terms, the behavior to preserve, and what is out of scope. The build reads the spec from the plan and opens the Figma link for the full detail; there is no separate catalog to point to.

## What never to do

1. While following this skill, do not edit code files: `.js`, `.css`, `.html`, `manifest.json`, or any other non-Markdown source file. Produce the build plan and stop; the build runs after the user pastes the plan back.
2. Do not edit CHANGELOG.md. It is updated during the build, after implementation.
